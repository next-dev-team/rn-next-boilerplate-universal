...
async function normalizeOptions(
  ...
  const hookFilename = options.pascalCaseFiles
    ? 'use'.concat(className) + 'Store'
    : 'use-'.concat(fileName) + 'Store';
  const hookName = 'use'.concat(className) + 'Store';
  const hookTypeName = 'IUse'.concat(className) + 'Store';
  ...
  const directory = await getDirectory(host, options, options.name);
...
async function getDirectory(host: Tree, options: Schema, baseHookName: string) {
