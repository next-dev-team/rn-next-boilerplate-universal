import { ColorStyleKey } from '@studio-ghibli-search-engine/models';
...
const tw = create(
  require('apps/studio-ghibli-search-engine-mobile/tailwind.config')
);
...
> = ({ children }) => {
  ...
  const [colorTwScheme, toggleColorScheme, setColorSchemeApp] =
    useAppColorScheme(tw);
  const twColor = (clx: string, clsKey?: ColorStyleKey) => {
    const cls = clx && (tw`${clx}` as any);
    const getColorKey =
      (clx.includes('bg-') && 'backgroundColor') ||
      (clx.includes('-') && 'color');
    if (!getColorKey) {
  ...
  const twGradients = (color1: string, color2: string, color3: string) => {
    ...
    return [
      ...
      tw.color(`bg-gray-400`),
      tw.color(`bg-gray-500`),
    ];
  };
